...
forEach metoda prolazi listu član po član (svaki član kao parametar) i na njemu radi određene izmjene. Te izmjenjene članove možemo spremiti u neku novu listu.
*/
...
const imenaUsera = useri.forEach ((user) => {
    return user;
});
console.log(imenaUsera); // forEach ne može spremiti podatke

/* 
FILTER metoda
Filter metoda za razliku od forEach vraća određenu vrijednost. Ta vrijednost je kopija dijela liste filtrirana na način da uključuje u tu novu listu samo one elemente koji zadovolje tražene parametre filtera
 */
...
// ista stvar korsiteći forEach metodu
...
// MAP metoda
// Map metoda nam omogućava da uzmemo jednu listu i iz nje napravimo drugu listu u kojoj će naši članovi liste izgledati malo drukčije
...
const lista = [
  ...
  // FIND metoda - pronalazi određenu vrijednost iz postojeće liste i sprema ih u novu listu - naći će samo prvu 

  const findLista = lista.find((auto) => auto.proizvod === "Automobil");

  console.log(findLista);

// SOME metoda - recimo da želimo saznati ima li koji proizvod cijenu manju od 150
